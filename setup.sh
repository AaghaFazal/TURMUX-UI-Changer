...
updatedw() {
	...
	then
		...
		if [[ \$var == *\"up to date\"* ]];
		then 
			clear && echo \"Already running the latest version!!\" && echo ------------------------------------- && figlet \$(sed '1q;d' 'ver.cfg') && figlet LgcyAlex
		else
			git pull
			bash setup.sh;
		...
		bash setup.sh
		wait
		echo \"Update Success\" && echo -------------- && figlet Success && figlet \$(sed '1q;d' '/data/data/com.termux/files/home/TURMUX-UI-Changer/ver.cfg') && figlet LgcyAlex && echo Restart to apply changes
	fi
	cd \$ppath
}
prm() { chmod +x *.\$1; }
...
if [ -d \"\$HOME/TURMUX-UI-Changer\" ]
then
	if grep -q '# 011' \"/data/data/com.termux/files/home/TURMUX-UI-Changer/.user.cfg\"
	then
		lnum=\$( sed '3q;d' \"/data/data/com.termux/files/home/TURMUX-UI-Changer/.user.cfg\" )
		lnum=\$( echo \$lnum | sed 's/# 011//g' )
		lnum=\$( echo \$lnum | sed 's/ //g' )
		echo \$( sed '3q;d' \"/data/data/com.termux/files/home/TURMUX-UI-Changer/.user.cfg\" )
			if [[ ! \$lnum -eq 5 ]]
			then
				lnum=\$((\$lnum+1))
				sed -i \"/.*# 011.*/ c\\ \$lnum # 011\" \"/data/data/com.termux/files/home/TURMUX-UI-Changer/.user.cfg\"
			else
				lnum=1
				sed -i \"/.*# 011.*/ c\\ \$lnum # 011\" \"/data/data/com.termux/files/home/TURMUX-UI-Changer/.user.cfg\"
				cd \$HOME
		...
		echo hi
		echo \"1 # 011\" >> \"/data/data/com.termux/files/home/TURMUX-UI-Changer/.user.cfg\"
	fi
else
	updatedw
fi
...
mdv README.md
cd $pdir
echo Find me on Telegram :-  @lgcyalex
echo tinyurl.com/LgcyAlex
